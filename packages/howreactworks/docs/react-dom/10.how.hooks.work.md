...
[`useRef`フック](https://react.dev/reference/react/useRef)は、任意の JavaScript 値を参照可能なミュータブルな ref オブジェクトを提供します。

公式ドキュメントによると：

> useRef はレンダリングに不要な値を参照するための React フックです

React はレンダー中に ref を使用して判断を下すことを推奨しませんが、レンダー外での操作は問題ありません。

例：

- レンダー中に ref 値を変更すると、StrictMode 下では複数回レンダーが発生し不正確な結果を招きます
- コンポーネントのマウント状態検知に ref を使用するのは誤り（Suspense フォールバック表示中もマウント状態は維持されます）
- HTML 要素に ref を渡すと、layout effect 時に実際の DOM 要素がアタッチされます

これらの理由から、レンダー中の ref 使用は最小限に抑え、公式ドキュメントで正しい使用方法を確認することを推奨します。
...
`useRef`を使用するコンポーネントが初めてレンダリングされるとき、[`mountRef`関数](https://github.com/facebook/react/blob/77c4ac2ce88736bbdfe0b29008b5df931c2beb1e/packages/react-reconciler/src/ReactFiberHooks.js#L2257)が呼び出されます。これは最もシンプルなフックの実装例です：

```tsx
// 簡略化版：レンダー中のref読み取りに関する開発警告は削除
function mountRef<T>(initialValue: T): { current: T } {
  // ステップ1
  // highlight-next-line
  const hook = mountWorkInProgressHook();
...
- **ステップ 1**: マウント時に hook オブジェクトを作成
- **ステップ 2**: `current`プロパティを持つ JavaScript オブジェクトを初期値で生成
- **ステップ 3**: このオブジェクトを hook の`memoizedState`に保存
- **ステップ 4**: ref オブジェクトを返却

### アップデート時の実装

アップデート時、`useRef`は[`updateRef`関数](https://github.com/facebook/react/blob/77c4ac2ce88736bbdfe0b29008b5df931c2beb1e/packages/react-reconciler/src/ReactFiberHooks.js#L2326)を呼び出します。実装は非常にシンプルです：
...
この処理では hook オブジェクトを再利用し、保存済みの mutable な ref オブジェクトをそのまま返します。これにより、前回の ref オブジェクトが保持され、同一性が維持されます。

hook オブジェクトを作成し、mutable な ref オブジェクトをそのまま再利用して、
保存されている状態を直接返却するだけのシンプルな実装です。
