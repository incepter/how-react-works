...
:::note
Having the `unstable_` prefix means that this option is still under development
or is experimental. Once stable and documented in React docs themselves, the
`unstable_` prefix will be removed and there are slight chances that even
the name may change too. So, avoid experimental and unstable APIs unless
you are fully confident that you can manage it (don't forget to leave some
comments too to instruct the next developer).
:::
...
todo: rewrite this section for the better, once the root is discussed very well
...
### 3. Declare variables mirroring the [provided `options`](https://github.com/facebook/react/blob/80d9a40114bb43c07d021e8254790852f450bd2b/packages/react-dom/src/client/ReactDOMRoot.js#L199).
   ```tsx
   // simplified
...
### 4. Call `createContainer` with the information in scope:
   ...
   for the `initialChildren` and `false` for hydrate (obviously at this point).
   ...
   2. Create the first `Fiber` or kind `HostRoot`:
...
### 6. Inject the current `ReactDispatcher`
   ...
   But to give you an idea, depending on the execution context (react current work,
   react currently rendering component, and so on) it will have a special object
   with many methods that the current code path expect them to be there.
   
   For example, at this point, we attach the `ReactDOMClientDispatcher` which
   is [defined here](https://github.com/facebook/react/blob/e50531692010bbda2a4627b07c7c810c3770a52a/packages/react-dom-bindings/src/client/ReactFiberConfigDOM.js#L2067).
...
So this is the final step in `createRoot`, finally!
...
Besides of the `_internalRoot` property, which is documented in the React docs
and should not be used, there two methods: `render` and `unmount`.
