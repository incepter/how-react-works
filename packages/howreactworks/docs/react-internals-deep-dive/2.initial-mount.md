...
As mentioned in [Overview of React internals](link-to-overview), `performConcurrentWorkOnRoot()` is the entry point to start rendering for both initial mount and re-render.
...
> ℹ️ Refer to [What are Lanes in React](link-to-lanes) to know more about Lanes
...
function workLoopSync() {
  ...
  while (workInProgress !== null) {
    // <This while loop means if workInProgress exists, it'll keep on performUnitOfWork() on it>
...
In order to understand this section more easily, I recommend you first check out my episode - [How does React traverse Fiber tree internally](link-to-fiber-tree-traversal).
...
function performUnitOfWork(unitOfWork: Fiber): void {
  ...
  if (next === null) {
    ...
    workInProgress = next;
    // <as mentioned, workLoopSync() is just a while loop that keeps running completeUnitOfWork() on workInProgress.  So assigning workInProgress here means setting next Fiber Node to work on>
  }
...
function beginWork(
  ...
  switch (workInProgress.tag) {
    ...
    case IndeterminateComponent: {
      // <IndeterminateComponent means the Class component or Function component that hasn't been instantiated yet. Once it is rendered it is determined with a right tag. We'll come back to this soon>
