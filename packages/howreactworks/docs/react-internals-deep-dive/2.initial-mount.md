...
[How does React traverse Fiber tree internally](/react/2022/01/16/fiber-traversal-in-react/)
を参照してください。
...
しかし、この関数はハイドレーション処理以外何も行いません。`<a>`と`<button>`のテキスト処理はコミットフェーズで行われます。

### 3.13 DOM ノードは`completeWork()`でオフスクリーン作成される

[How does React traverse Fiber tree internally](/react/2022/01/16/fiber-traversal-in-react/)で説明した通り、
...
[React 内部の概要](/2023-07-11-overall-of-react-internals)で簡単に説明したコミットフェーズについて、
