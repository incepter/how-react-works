...
Episode 3 from [React Internals Deep Dive](/series/react-source-code-walkthrough.html), watch me [explaining this on Youtube](https://www.youtube.com/watch?v=0GM-1W7i9Tk&list=PLvx8w9g4qv_p-OS-XdbB3Ux_6DMXhAJC3&index=3)
...
I’ve explained [how React does the initial mount](/2023-07-14-initial-mount/) and
creates the full DOM from scratch. For re-renders after initial mount, React tries
to reuse the DOM as much as possible through the process of reconciliation. In this
episode, we’ll figure out what actually happens when React re-renders after clicking
the button in demo below.

The demo is different from previous episode. You can [try it out here](/demos/react/overview/re-render.html).

## Table of Contents

- [1. Re-render in Trigger phase](#1-re-render-in-trigger-phase)
  - [1.1 `lanes` and `childLanes`](#11-lanes-and-childlanes)
- [2. Re-render in Render phase.](#2-re-render-in-render-phase)
  - [2.1 Basic rendering logic is the same as initial mount.](#21-basic-rendering-logic-is-the-same-as-initial-mount)
  - [2.2 React reuses redundant Fiber Nodes before creating new ones](#22-react-reuses-redundant-fiber-nodes-before-creating-new-ones)
  - [2.3 The Update branch in `beginWork()`](#23-the-update-branch-in-beginwork)
  - [2.4 Bailout logic inside `attemptEarlyBailoutIfNoScheduledUpdate()`](#24-bailout-logic-inside-attemptearlybailoutifnoscheduledupdate)
  - [2.5 `memoizedProps` vs `pendingProps`.](#25-memoizedprops-vs-pendingprops)
  - [2.6 `updateFunctionComponent()` re-render function components and reconcile children.](#26-updatefunctioncomponent-re-renders-function-components-and-reconcile-children)
  - [2.7 `reconcileSingleElement()`.](#27-reconcilesingleelement)
  - [2.8 Once a component is re-rendered, their subtree is re-rendered by default.](#28-once-a-component-is-re-rendered-their-subtree-is-re-rendered-by-default)
  - [2.9 `updateHostComponent()`.](#29-updatehostcomponent)
  - [2.10 `reconcileChildrenArray()` creates and deletes fibers as needed.](#210-reconcilechildrenarray-creates-and-deletes-fibers-as-needed)
  - [2.11 `placeChild()` and `deleteChild()` marks fiber with flags](#211-placechild-and-deletechild-marks-fiber-with-flags)
  - [2.12 `updateHostText()`](#212-updatehosttext)
  - [2.13 `completeWork()` marks the update of HostComponent and creates DOM nodes if necessary](#213-completework-marks-the-update-of-hostcomponent-and-creates-dom-nodes-if-necessary)
- [3. Re-render in Commit Phase](#3-re-render-in-commit-phase)
  - [3.1 `commitMutationEffectsOnFiber()` kicks of the commit of Insertion/Deletion/Update](#31-commitmutationeffectsonfiber-kicks-off-the-commit-of-insertiondeletionupdate)
  - [3.2 Deletion are processed first, before processing children and self.](#32-deletion-are-processed-first-before-processing-children-and-self)
  - [3.3 Insertions are processed next](#33-insertions-are-processed-next)
  - [3.4 Updates are handled the last](#34-updates-are-handled-at-last)
- [4. Summary](#4-summary)

## 1. Re-render in Trigger phase

React constructs the Fiber Tree and DOM tree in [initial mount](/2023-07-14-initial-mount/),
when it’s done we have two trees as below.
...
1.  `lanes` => for the pending work of itself
2.  `childLanes` => for the pending work of its subtree

For more details on Lanes, please refer to [What are Lanes in React source code](/react/2022/03/26/lanes-in-react/)

When the button is clicked, `setState()` is called:

1.  the path from root to the target fiber
    will be marked with `lanes` and `childLanes` to indicate places that need to be checked in next render.
2.  an update is scheduled by `scheduleUpdateOnFiber()`, in which eventually `ensureRootIsScheduled()` is called
    and `performConcurrentWorkOnRoot()` is scheduled in Scheduler. This is quite similar to the [initial mount](/2023-07-14-initial-mount/).

One important thing to keep in mind is that priority of event determines the priority
of the update. For `click` event, it is `DiscreteEventPriority`, which maps to `SyncLane` - high priority.

For more details on how `useState()` works, refer to [How does useState() work internally in React](/2023-06-19-how-does-usestate-work).

We’ll skip the details here but eventually we get following Fiber Tree to work on.
...
Since under `click` event, the rendering lane is SyncLane which is blocking lane.
So same as initial mount, concurrent mode is still not activated inside `performConcurrentWorkOnRoot()`.

For cases that concurrent mode is on, refer to [How does useTransition() work internally in React?](/2023-05-19-how-does-usetransition-work/)
...
function performUnitOfWork(unitOfWork: Fiber): void {
  ...
  // ~This line is important, we'll cover it in [2.5 memoizedProps vs pendingProps](#25-memoizedprops-vs-pendingprops)
...
Refer to my [previous episode](/2023-07-14-initial-mount/#32-renderrootsync) for better explanation.
Here just remember that React traverse the Fiber Tree and updates the fiber if necessary.

### 2.2 React reuses redundant Fiber Nodes before creating new ones

In [initial mount](/2023-07-14-initial-mount/) we see that Fibers are created
from scratch. But actually React tries to reuse the Fiber Nodes first.

```javascript
export function createWorkInProgress(current: Fiber, pendingProps: any): Fiber {
  let workInProgress = current.alternate;
  // ~curernt is the current version
  ...
  // ~lanes and childLanes are copied
...
Since FiberRootNode points to the current Fiber Tree through `current`, every
Fiber Node that is not on the current tree could be reused.

In our re-render process, the redundant `HostRoot` will be reused in `prepareFreshStack()`.
...
In `beginWork()`, there is an important branch that handles updates, which we didn’t
...
1.  if a fiber doesn’t have props/context changes and it doesn’t have pending work (empty `lanes`)
    1.  if its children don’t have pending work(empty `childLanes`),
        bailout happens and React doesn’t go deeper in the tree.
...
For more on bailout, refer to [How does React bailout work in reconciliation](/react/2022/01/07/how-does-bailout-work/).

### 2.5 `memoizedProps` vs `pendingProps`.

In `beginWork()`, `workInProgress` is diffed against `current`. For the props,
it is `workInProgress.pendingProps` against `current.memoizedProps`. We can
think of `memoizedProps` as current props while `pendingProps` as next version.

React creates a new Fiber Tree in Render phase and then diff against current
Fiber Tree. We can see that `pendingProps` is actually a parameter for workInProgress
creation.
...
Which makes sense, creating Fiber Node is the very first step. It needs
to be worked on later.

And `memoizedProps` is set with `pendingProps` when re-rendering for a fiber
...
1.  React works on HostRoot(lanes: 0, childLanes: 1). HostRoot doesn’t have props and
    `memoizedProps` and `pendingProps` are both null, so React goes straight to its child, which is
    the cloned `App`.
2.  React works on `<App/>`(lanes: 0, childLanes: 1). App component is not re-rendered so
    `memoizedProps` and `pendingProps` are the same, so React goes straight to its child, which is
    the cloned `div`.
3.  React works on `<div/>`(lanes: 0, childLanes: 1). It gets its children from App, but App is not
    re-run so none of its children(`<Link>`, `<br/>` and `<Component/>`) is changed, thus
    again React goes straight to `<Link/>`.
4.  React works on `<Link/>`(lanes: 0, childLanes: 0). This time React doesn’t even need to go deeper,
    so it stops here and goes to its sibling - `<br/>`.
5.  React works on `<br/>`(lanes: 0, childLanes: 0), bailout happens again, React goes to `<Component/>`

Now something is a bit different. `<Component/>` has `lanes` of `1` meaning React has to
...
We’ve met `reconcileChildren()` in [how React does the initial mount](/2023-07-14-initial-mount/).
